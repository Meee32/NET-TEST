...
unsigned int nStakeSplitAge = 1 * 12 * 60 * 60;
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %snet %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %snet %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() == 0  && pcoin->GetDepthInMainChain() == nCoinbaseMaturity+20)
            {
                ...
                if (nBestHeight <= nPrevS4CHeight ) {
                    ...
                    SendMoneyToDestination(strStakeForCharityAddress.Get(), nNet, sNarr, wtx, false, true);
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, bool fAllowS4C, int32_t& nChangePos, const CCoinControl* coinControl)
{
    int64_t nValue = 0;
...
bool CWallet::CreateStealthTransaction(CScript scriptPubKey, int64_t nValue, std::vector<uint8_t>& P, std::vector<uint8_t>& narr, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, bool fAllowS4C, const CCoinControl* coinControl)
{
    ...
    int nChangePos;
    bool rv = CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, fAllowS4C, nChangePos, coinControl);
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, bool fAllowS4C, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Calculate coin age reward
    {
        ...
        int64_t nCoinValue;
        CTxDB txdb("r");
        if (!txNew.GetCoinAge(txdb, txNew.nTime, nCoinAge, nCoinValue))
            return error("CreateCoinStake : failed to calculate coin age");

        int64_t nReward = GetProofOfStakeReward(nCoinAge, nCoinValue, nFees, pindexPrev->nHeight);
        if (nReward <= 0)
            return false;
...
string CWallet::SendMoney(CScript scriptPubKey, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, bool fAllowS4C)
{
    CReserveKey reservekey(this);
    int64_t nFeeRequired;
    ...
    if (!CreateTransaction(scriptPubKey, nValue, sNarr, wtxNew, reservekey, nFeeRequired, fAllowS4C))
    {
        string strError;
        if (nValue + nFeeRequired > GetBalance())
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, bool fAskFee, bool fAllowS4C)
{
    // Check amount
    if (nValue <= 0)
    ...
    return SendMoney(scriptPubKey, nValue, sNarr, wtxNew, fAskFee, fAllowS4C);
}
...
{
    {
        ...
        while (setKeyPool.size() < (nTargetSize + 1))
        {
            ...
            LogPrint("keypool", "keypool added key %d, size=%"PRIszu"\n", nEnd, setKeyPool.size());
        }
    }
    return true;
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, int& nOrphansFound, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %snet %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %snet %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool CWalletManager::LoadWallet(const string& strName, ostringstream& strErrors, bool fRescan, bool fUpgrade, bool fZapWallet, int nMaxVersion)
{
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading ") << strFile << _(": Wallet requires newer version of Netcoin") << "\n";
...
bool CWalletManager::LoadWalletFromFile(const string& strFile, string& strName, ostringstream& strErrors, bool fRescan, bool fUpgrade, bool fZapWallet, int nMaxVersion)
{
    ...
    {
       if (!NewThread(ThreadStakeMiner, pWallet))
          LogPrintf("Error: NewThread(ThreadStakeMiner) failed\n");
    }
    else
      LogPrintf("Skipped ThreadStakeMiner for wallet: %s due to encryption\n", pWallet->strWalletFile);
       ...
       if (!fShutdown)
       {
         ...
         for (unsigned int i = 0; i < vstrNames.size(); i++)
         {
             if ( !vpWallets[i].get()->IsCrypted() || !vpWallets[i].get()->IsLocked() )
              {
                 LogPrintf ("Restarting ThreadStakeMiner for: %s\n", vstrNames[i]);
                 if (!NewThread(ThreadStakeMiner, vpWallets[i].get()))
                    LogPrintf("Error: NewThread(ThreadStakeMiner) failed\n");
              }
              else
                LogPrintf("Skipped ThreadStakeMiner for wallet: %s due to encryption\n", vstrNames[i]);
