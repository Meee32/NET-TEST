...
double getTxTotalValue2(std::string txid)
{
    uint256 hash;
    hash.SetHex(txid);
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        buffer = value + convertCoins2(txout.nValue);
        value = buffer;
    }
...
double getMoneySupply(qint64 Height)
{
    ...
    CBlockIndex* pblockindex = mapBlockIndex[hash];
    return convertCoins2(pblockindex->nMoneySupply);
}

double convertCoins2(qint64 amount)
{
    // Tranz needs to use options model.
    return (double)amount / (double)COIN;
}

std::string getOutputs2(std::string txid)
{
    uint256 hash;
    hash.SetHex(txid);
    ...
    for (unsigned int i = (tx.IsCoinStake() ? 1 : 0); i < tx.vout.size(); i++)
    {
        ...
        double buffer = convertCoins2(txout.nValue);
...
std::string getInputs2(std::string txid)
{
    uint256 hash;
    hash.SetHex(txid);
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        double buffer = convertCoins2(wtxPrev.vout[vin.prevout.n].nValue);
        std::string amount = boost::to_string(buffer);
        str.append(CBitcoinAddress(address).ToString());
        str.append(": ");
        str.append(amount);
...
double BlockBrowser::getTxFees(std::string txid)
{
    ...
    if (!GetTransaction(hash, tx, hashBlock))
        return convertCoins2(MIN_TX_FEE);
...
void BlockBrowser::updateExplorer(bool block)
{
    if(block)
    {
        ...
        ui->valueBox->setText(QString::number(getTxTotalValue2(txid), 'f', 6) + " HBN");
        ui->txID->setText(QString::fromUtf8(txid.c_str()));
        ui->outputBox->setText(QString::fromUtf8(getOutputs2(txid).c_str()));
        ui->inputBox->setText(QString::fromUtf8(getInputs2(txid).c_str()));
