...
void ThreadStakeMiner(void* parg)
{
    ...
    LogPrintf("ThreadStakeMiner started for wallet: %s\n", pwallet->strWalletFile );
    try
    {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Netcoin is probably already running."), addrBind.ToString());
        else
