...
Value sendtoaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <netcoinaddress> <amount> [narration] [comment] [comment-to]\n"
    ...
    string strError = pWallet->SendMoneyToDestination(address.Get(), nAmount, sNarr, wtx, false, false);
    if (strError != "")
...
int64_t GetAccountBalance(CWallet* pWallet, CWalletDB& walletdb, const string& strAccount, int nMinDepth)
{
    int64_t nBalance = 0;
...
int64_t GetAccountBalance(CWallet* pWallet, const string& strAccount, int nMinDepth)
{
    CWalletDB walletdb(pWallet->strWalletFile);
...
Value getbalance(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    int64_t nBalance = GetAccountBalance(pWallet, strAccount, nMinDepth);
...
Value sendfrom(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <tonetcoinaddress> <amount> [minconf=1] [narration] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase(pWallet));
...
Value sendmany(CWallet* pWallet, const Array& params, bool fHelp)
{
    ...
    int nChangePos;
    bool fCreated = pWallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, false, nChangePos, false);
    if (!fCreated)
    {
...
Value getnewstealthaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
...
Value liststealthaddresses(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
    ...
    for (it = pWallet->stealthAddresses.begin(); it != pwalletMain->stealthAddresses.end(); ++it)
    {
...
Value importstealthaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2)
        throw runtime_error(
...
Value sendtostealthaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
...
Value clearwallettransactions(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 0)
        throw runtime_error(
    ...
    
    {
        ...
        while (true)
        {
            ...
            if (strType == "tx")
            {
                ...
                pWallet->NotifyTransactionChanged(pwalletMain, hash, CT_DELETED);
...
Value scanforalltxns(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
...
Value scanforstealthtxns(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
    ...
    while (pindex)
    {
        ...
        BOOST_FOREACH(CTransaction& tx, block.vtx)
        {
            ...
            pWallet->AddToWalletIfInvolvingMe(tx, &block, fUpdate);
        };
