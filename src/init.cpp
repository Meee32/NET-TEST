...
{
    uiInterface.ThreadSafeMessageBox(str, _("Netcoin"), CClientUIInterface::MSG_ERROR);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("Netcoin"), CClientUIInterface::MSG_WARNING);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 30915 or testnet: 30916)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 20913 or testnet: 20914)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Netcoin is probably already running."), strDataDir));
