...
unsigned int nStakeMaxAge       = 25920;          // 30 Days
...
const string strMessageMagic = "ShadowCoin Signed Message:\n";
...
int64_t nSplitThreshold = GetProofOfWorkReward(0, 0, 0);
int64_t nCombineThreshold = GetProofOfWorkReward(0, 0, 0) * 2;
...
{
    nCoinValue = pwalletMain->GetBalance();
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nCoinValue,  int64_t nFees, int64_t nHeight)
{
    ...
    if (fDebug && GetBoolArg("-printcreation"))
        LogPrint("creation", "GetProofOfStakeReward(): PIR=%.1f create=%s nCoinAge=%d nCoinValue=%s nFees=%d\n", (double)nRewardCoinYear/(double)CENT, FormatMoney(nSubsidy), nCoinAge, FormatMoney(nCoinValue), nFees);
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            uint64_t nCoinAge;
            int64_t nCoinValue;
            if (!GetCoinAge(txdb, nTime, nCoinAge, nCoinValue))
            ...
            int64_t nHeight;
            int64_t nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, nCoinValue, nFees, nHeight) - GetMinFee() + MIN_TX_FEE;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        LogPrintf("*** %s\n", strMessage);
        uiInterface.ThreadSafeMessageBox(strMessage, "Netcoin", CClientUIInterface::MSG_WARNING);
        StartShutdown();
        return false;
    }
    return true;
